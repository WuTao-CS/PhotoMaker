...
def parse_args():
    ...
    parser.add_argument("--enable_reg_loss", action="store_true")
    parser.add_argument("--reg_loss_weight", type=float, default=0.3, help="Weight decay to use.")
...
def main(args):
    ...
    train_dataset = CelebVTextSD15Dataset(args.train_data_dir, get_latent_from_video=args.with_vae, video_length=args.video_length, resolution=[args.resolution,args.resolution],frame_stride=args.frame_stride,image_drop_ratio=args.image_drop_ratio)
    print("######## image drop ratio ###########")
    print(args.image_drop_ratio)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                origin_noisy_latents = noisy_latents
                ...
                if args.enable_reg_loss:
                    cross_attention_kwargs={'use_origin_attn':True}
                    reg_pred = unet(origin_noisy_latents, timesteps, encoder_hidden_states, return_dict=False,cross_attention_kwargs=cross_attention_kwargs)[0]
                    loss_reg = F.mse_loss(reg_pred.float(), target.float(), reduction="mean")
                    loss = loss + args.reg_loss_weight * loss_reg
