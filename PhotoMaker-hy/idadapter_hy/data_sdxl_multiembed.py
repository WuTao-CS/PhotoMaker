...
org_str = "/arc_org_data/"
dst_str = "/arc_org_data_caption/"
...
class IDAdapterFileListDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        target_json_path = image_path.replace(".png", ".json").replace(org_str, dst_str)
        object_image_paths = list(map(lambda x: x.replace(".mask.png", ".png"), object_mask_paths))
        object_json_paths = list(map(lambda x: x.replace(".mask.png", ".json").replace(org_str, dst_str), object_mask_paths))
...
class IDAdapterFFHQDataset(torch.utils.data.Dataset):
    ...
    def __getitem__(self, idx):
        ...
        info_path = os.path.join(self.root, chunk, image_id + ".json").replace(org_str, dst_str)
...
def get_data_loader(dataset, batch_size, shuffle=True, debug=False):
...
if __name__ == "__main__":
    ...
    args = parse_args()
    args.pretrained_model_name_or_path = "stabilityai/stable-diffusion-xl-base-1.0"
    args.dataset_name = "/apdcephfs_cq2/share_1367250/nicehuang/data/photomaker/arc_org_data"
