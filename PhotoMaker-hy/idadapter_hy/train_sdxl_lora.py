...
def train():
    ...
    print("-------------")
    print("lora num: ", len(unet_lora_attn_procs))
    print("-------------")
    ...
    if args.load_model is not None:
        if os.path.exists(Path(args.load_model) / "pytorch_model.bin"):
            model.load_state_dict(
                torch.load(Path(args.load_model) / "pytorch_model.bin", map_location="cpu", strict=False)
            )
        elif os.path.exists(Path(args.load_model) / "model.safetensors"):
            model.load_state_dict(
                torch.load(Path(args.load_model) / "model.safetensors", map_location="cpu", strict=False)
            )       
        else:
        ...
        if args.use_hunyuan_image_encoder:
            if args.image_encoder_trainable_layers > 0:
                for idx in range(args.image_encoder_trainable_layers):
                    model.image_encoder.vision_model.model.visual.transformer.resblocks[-1-idx].requires_grad_(True)
                    model.image_encoder.vision_model.model.visual.transformer.resblocks[-1-idx].to(
    ...
    encoder_params = list(
        [p for n, p in model.image_encoder.named_parameters() if p.requires_grad]
    )
    # encoder_params = list(
    ...
    postfuse_params = list(
        [p for n, p in model.postfuse_module.named_parameters() if p.requires_grad]
    )
    # postfuse_params = list(
    ...
    parameters = unet_lora_parameters + encoder_params + postfuse_params
    ...
    if args.large_step_postfuse:
        optimizer = optimizer_cls(
            [
                {"params": unet_lora_parameters  + postfuse_params, "lr": args.learning_rate * args.unet_lr_scale}, # TODO: 
        ...
        optimizer = optimizer_cls(
            [
                {"params": unet_lora_parameters, "lr": args.learning_rate * args.unet_lr_scale}, # TODO: 
    ...
    train_dataloader = get_data_loader(train_dataset, args.train_batch_size, debug=args.debug)
            ...
            for step, batch in enumerate(train_dataloader):
                ...
                with accelerator.accumulate(model), torch.backends.cuda.sdp_kernel(
                    ...
                    with torch.no_grad():
                        ...
                        # print(latents.shape)
                        prompt_embeds, pooled_prompt_embeds = model.module.encode_prompt(batch["input_ids"])
                ...
                if accelerator.sync_gradients:
                    ...
                    if (
                        ...
                        if args.keep_only_last_checkpoint and accelerator.is_main_process:
