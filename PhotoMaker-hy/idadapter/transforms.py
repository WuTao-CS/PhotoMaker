...
# class ObjectSegmapTransform(torch.nn.Module):
...
def get_object_processor(args):
    if args.object_background_processor == "random":
        ...
        raise ValueError(f"Unknown object processor: {args.object_background_processor}")
    return object_processor

# def get_object_segmap_processor(args):
#     object_segmap_processor = ObjectSegM
#     return object_segmap_processor

def tensor_to_image(tensor, filename="dump_data/idadapter/tensors.jpg", norm=True):
    if norm:
        tensor = (tensor + 1) * 127.5
    pic = T.functional.to_pil_image(tensor.to(torch.uint8), mode=None)
    pic.save(filename)