...
import diffusers
from diffusers import AutoencoderKL,AnimateDiffPipeline, DDIMScheduler, DDPMScheduler, EulerDiscreteScheduler, MotionAdapter, UNet2DConditionModel, UNetMotionModel
...
from model.datasets.celebv_text import CelebVTextSD15Dataset,CelebVTextSD15NewFaceDataset
...
def parse_args():
    ...
    parser.add_argument(
        "--new_data", action="store_true", help="Whether or not to update motion layer."
    )
...
def main(args):
    ...
    for name, param in unet.named_modules():
        if args.enable_update_motion and "motion_modules" in name and 'attn1' in name:
    ...
    if args.new_data:
        train_dataset = CelebVTextSD15NewFaceDataset(args.train_data_dir, get_latent_from_video=args.with_vae, video_length=args.video_length, resolution=[args.resolution,args.resolution],frame_stride=args.frame_stride)
    else:
