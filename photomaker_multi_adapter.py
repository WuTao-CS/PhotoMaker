...
input_folder_name = 'datasets/test_dataset_cvpr24'
for dir_name in os.listdir(input_folder_name):
    image_basename_list =[base_name for base_name in os.listdir(os.path.join(input_folder_name,dir_name)) if isimage(base_name)]
    image_path_list = sorted([os.path.join(input_folder_name, dir_name, basename) for basename in image_basename_list])
