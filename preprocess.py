...
class Annotator(nn.Module):
    ...
    def forward(self, batch, ref_data=None):
        ...
        if ref_data is not None:
            prompt_embeds_trigger = ref_data["prompt_embeds_trigger"].unsqueeze(dim=0).to(self.device)
            class_tokens_mask = ref_data["class_tokens_mask"].unsqueeze(dim=0).to(self.device)
            ...
            ref_data["prompt_embeds_trigger"] = prompt_embeds_trigger.squeeze(dim=0)
            return ref_data
        else:
