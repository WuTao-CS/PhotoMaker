...
import diffusers
from diffusers import AutoencoderKL, DDIMScheduler, DDPMScheduler, EulerDiscreteScheduler, MotionAdapter, UNet2DConditionModel, UNetMotionModel, AnimateDiffSDXLPipeline, AnimateDiffPipeline
...
def main(args):
    ...
    for name, param in unet.named_modules():
        ...
        elif "motion_modules" not in name and "attn2" in name:
            if isinstance(param, (nn.Linear, nn.LayerNorm)):
                lora_target_modules.append(name)
    ...
    init_token_embeds = torch.load('bust_shot_sd15_emb.pt',map_location='cpu')
    setattr(unet, "inject_learnable_token",torch.nn.Parameter(init_token_embeds, requires_grad=True))
    if args.with_vae:
    ...
    param_to_learn = filter(lambda p: p.requires_grad, unet.parameters())
    # Optimizer creation
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                encoder_hidden_states = torch.cat([encoder_hidden_states[:,:1,:], unet.inject_learnable_token.repeat(bsz, 1, 1), encoder_hidden_states[:,1:,:]], dim=1)
            ...
            if accelerator.sync_gradients:
                ...
                if accelerator.distributed_type == DistributedType.DEEPSPEED or accelerator.is_main_process:
                    if global_step % args.checkpointing_steps == 0:
                        ...
                        torch.save(unet.inject_learnable_token.data, os.path.join(save_path,'learnable_token.pt'))
