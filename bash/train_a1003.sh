...
#   --snr_gamma 5.0
CUDA_VISIBLE_DEVICES=4,5,6,7 accelerate launch --config_file 'bash/accelerate_config_4a100.yaml' \
  --main_process_port 14231 \
  train_latent_gate_skipt.py \
  --pretrained_model_name_or_path="./pretrain_model/stable-diffusion-v1-5" \
  --train_data_dir "datasets/CeleV-Text" \
  --checkpointing_steps=1000 \
  --resolution 512 \
  --output_dir "checkpoints/debug_sd15_gate_latent_lr_1e-5_skipt-a100" \
  --checkpoints_total_limit=10 \
  --num_train_epochs=100 \
  --train_batch_size=1 \
