...
def expand_first(feat: T, scale=1.,) -> T:
    b = feat.shape[0]
    # old
    # num_videos = (b // 16) // 2 # num_frames
    # feat_style = torch.cat([feat[0:16].repeat(num_videos,1,1,1), feat[b // 2:b // 2 + 16].repeat(num_videos,1,1,1)],dim=0)
    # new
    num_videos = 2
    num_frames = b // 2 // num_videos
    feat_style = torch.cat([feat[0:num_frames].repeat(num_videos,1,1,1), feat[b // 2:b // 2 + num_frames].repeat(num_videos,1,1,1)],dim=0)
    
    # feat_style = torch.stack((feat[0:16], feat[b // 2:b // 2 + 16])).unsqueeze(1)
    # if scale == 1:
    #     feat_style = feat_style.expand(2, b // 2, *feat_style.shape[1:])
    # else:
...
def adain(feat: T, is_motion=False) -> T:
    feat_mean, feat_std = calc_mean_std(feat) # 每一帧所有像素点的均值 和 方差
    ...
    if is_motion:
        ...
        feat = feat * feat_std + feat_style_mean
    return feat
...
class SharedAttentionProcessor(DefaultAttentionProcessor):
    ...
    def shared_call(
        ...
        if self.adain_queries:
            query = adain(query,self.is_motion)
        if self.adain_keys:
            key = adain(key,self.is_motion)
        if self.adain_values:
            value = adain(value,self.is_motion)
        ...
        if self.share_hidden:
            hidden_states = adain(hidden_states,self.is_motion)
    ...
    def __init__(self, style_aligned_args: StyleAlignedArgs, is_motion=False):
        super().__init__()
        ...
        self.share_hidden = style_aligned_args.adain_hidden
...
def init_attention_processors(pipeline: AnimateDiffPipeline, style_aligned_args: StyleAlignedArgs | None = None):
    attn_procs = {}
    unet = pipeline.unet
    ...
    for i, name in enumerate(unet.attn_processors.keys()):
        is_self_attention = 'attn1' in name and "motion_modules" not in name
        is_motion_attention = "motion_modules" in name
        if is_self_attention:
            number_of_self += 1
            if style_aligned_args is None or only_self_vec[i // 2] or style_aligned_args.with_self_attn is False:
                attn_procs[name] = DefaultAttentionProcessor()
            else:
                attn_procs[name] = SharedAttentionProcessor(style_aligned_args)
        elif is_motion_attention:
            number_of_temp+=1
            if style_aligned_args.with_motion_layer:
                attn_procs[name] =  SharedAttentionProcessor(style_aligned_args,is_motion=True)
            else:
                attn_procs[name] = unet.attn_processors[name]
        else:
            number_of_cross += 1
            attn_procs[name] = unet.attn_processors[name]
    print(f"Number of self attention layers: {number_of_self}, number of cross attention layers: {number_of_cross}", f"number of temp attention layers: {number_of_temp}")
    unet.set_attn_processor(attn_procs)


def register_shared_norm(pipeline: AnimateDiffPipeline,
                         share_group_norm: bool = True,
                         share_layer_norm: bool = True, ):
...
class Handler:
    ...
    def __init__(self, pipeline: AnimateDiffPipeline):
