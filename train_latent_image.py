...
import diffusers
from diffusers import AutoencoderKL, DDIMScheduler, DDPMScheduler, EulerDiscreteScheduler, MotionAdapter, UNet2DConditionModel, UNetMotionModel, StableDiffusionPipeline
...
from model.datasets.ffhq import FFHQDataset
try:
    import torch_npu
...
def collate_fn(data):
    ...
    ref_pixel_values = torch.stack([example["ref_pixel_values"] for example in data])
...
def main(args):
    ...
    train_dataset = FFHQDataset(root=args.train_data_dir, tokenizer=tokenizer)
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                with torch.no_grad():
                    ...
                    batch["ref_pixel_values"].to(dtype=torch.float32)
                    
                    # concat pixel_values and ref_pixel_values
                    input = torch.cat([batch["pixel_values"], batch["ref_pixel_values"]], dim=0)
                    
                    batch["pixel_values"] = vae.encode(batch["pixel_values"]).latent_dist.sample() * vae.config.scaling_factor
                    batch["ref_pixel_values"] = vae.encode(batch["ref_pixel_values"]).latent_dist.sample() * vae.config.scaling_factor
                    
                latents = batch["pixel_values"].to(weight_dtype)
                ref_latents = batch["ref_pixel_values"].to(weight_dtype)
                ...
                if args.enable_reference_noisy:
                    ...
                    ref_timesteps = torch.tensor((timesteps * args.ref_noisy_ratio),dtype=torch.int)
                ...
                model_pred, ref_pred = torch.chunk(model_pred, chunks=2, dim=2)
