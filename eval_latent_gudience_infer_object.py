...
def get_parser(**parser_kwargs):
    ...
    parser.add_argument("--image", type=str, default='datasets/object_benchmark_seg',help="image")
...
names = os.listdir(args.image)
image_path_list =[]
for name in names:
    text = name+'.'
    image_basename_list =[base_name for base_name in os.listdir(os.path.join(args.img_path,name)) if isimage(base_name)]
    for basename in image_basename_list:
        image_path_list.append(os.path.join(args.image, name, basename))
...
for image_path, input_id_image in zip(image_path_list, input_id_images):
    ...
    prompts = load_prompts(os.path.join(folder_path,"prompts.txt"))
