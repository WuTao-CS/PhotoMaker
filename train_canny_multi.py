...
from model.attention_processor import ReferVideoAttnProcessor2_0, NewLastFrameProjectAttnProcessor2_0, ReferSelectVideoAttnProcessor2_0
from model.datasets.celebv_text_canny import CelebVTextSelectCannySD15Dataset,CelebVCannySD15LatentDataset
...
def parse_args():
    ...
    parser.add_argument(
        "--enable_new_prompt", action="store_true", help="Whether or not to update motion layer."
    )
    parser.add_argument(
        "--refer_noisy_type",
        type=str,
...
def main(args):
    ...
    for name in unet.attn_processors.keys():
        ...
        if "motion_modules" in name:
            ...
            attn_procs[name] = ReferSelectVideoAttnProcessor2_0(
                ...
                num_frames=16,
                num_reference_frame=args.num_reference_frame,
            )
        else:
            attn_procs[name] = unet.attn_processors[name]                       
    ...
    if args.with_vae:
        train_dataset = CelebVTextSelectCannySD15Dataset(args.train_data_dir,new_prompt=args.enable_new_prompt,video_length=args.video_length, resolution=[args.resolution,args.resolution],frame_stride=args.frame_stride, num_reference_frame=args.num_reference_frame)
    else:
        train_dataset = CelebVCannySD15LatentDataset(args.train_data_dir,new_prompt=args.enable_new_prompt,video_length=args.video_length, resolution=[args.resolution,args.resolution],frame_stride=args.frame_stride)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                batch["video"] = batch["video"].to(weight_dtype)
                # Convert images to latent space
                latents = batch["video"][:,:,:16,:,:].to(weight_dtype)
                ref_latents = batch["video"][:,:,16:,:,:].to(weight_dtype)
