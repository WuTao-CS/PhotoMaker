...
from model.attention_processor import LastFrameProjectAttnProcessor2_0, SkipMotionAttnProcessor2_0, LastFrameGatedAttnProcessor2_0, SkipLastMotionAttnProcessor2_0, NewLastFrameProjectAttnProcessor2_0
...
class ReferenceDiffusion(torch.nn.Module):
    def __init__(self, unet):
        super().__init__()
        self.unet = unet
        self.latent_down = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=4, out_channels=1, kernel_size=3, stride=1, padding=1),
            torch.nn.ReLU(),
            torch.nn.Conv2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1),
            torch.nn.ReLU(),
            torch.nn.Conv2d(in_channels=1, out_channels=1, kernel_size=3, stride=2, padding=1),
            torch.nn.ReLU(),
        )
        self.fc_time = nn.Linear(320, 256)
        self.fc = nn.Linear(256, 1)
    
    def forward(self, latents, ref_latents, noise, timesteps, encoder_hidden_states, noise_scheduler, max_ref_noisy_ratio=1e-2):
        timesteps = timesteps.long()
        noisy_latents = noise_scheduler.add_noise(latents, noise, timesteps)
        timesteps_tmp = timesteps.expand(ref_latents.shape[0]).to(noisy_latents.dtype)
        ...
        ref_feature = ref_latents.squeeze(dim=2)
        ref_feature = self.latent_down(ref_feature)
        ref_feature = ref_feature.view(ref_feature.size(0), -1) + t_emb
        pred_time = self.fc(ref_feature)
        ratio = torch.mean(F.sigmoid(pred_time),dim=-1)
        ref_timesteps = torch.tensor(int(timesteps * max_ref_noisy_ratio * ratio),dtype=torch.int)
        ...
        return self.unet(noisy_latents, timesteps, encoder_hidden_states, return_dict=False)[0], noisy_latents
...
def main(args):
    ...
    model = ReferenceDiffusion(unet)
    params_to_optimize=[]
    for name, parm in model.unet.named_parameters():
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(model):
                ...
                model_pred, noisy_latents = model(latents, ref_latents, noise, timesteps, encoder_hidden_states, noise_scheduler, max_ref_noisy_ratio=args.ref_noisy_ratio)
