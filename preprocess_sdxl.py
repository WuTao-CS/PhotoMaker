...
class Annotator(nn.Module):
    ...
    def forward(self, batch, ref_data=None):
        ...
        latent = vae_output[:,:video.shape[0],:,:,:]
        ref_latent = vae_output[:,video.shape[0]:,:,:,:]
...
if __name__ == "__main__":
    ...
    annotator.to("cuda")
    print(args.phase)
    data = CelebVTextProcessDataset(root=args.root,load_all_frames=False,phase=args.phase,total=args.total,resolution=[512,512])
    ...
    for batch in tqdm(data_loader):
        if batch == {}:
            continue
        ref_data = None
        if os.path.exists(f"{args.save_path}/processed_sdxl_512/{batch['name'][0]}.pt"):
            continue
        with torch.no_grad():
        ...
        save_path = f"{args.root}/processed_sdxl_512/{batch['name'][0]}.pt"
        all_data.append({"path":save_path, "prompt":batch['prompt'][0], "prompt_trigger":batch['prompt_trigger'][0]})
        torch.save(output, f"{args.save_path}/processed_sdxl_512/{batch['name'][0]}.pt")

    json_file_name = os.path.join(args.save_path, "processed_sdxl_512_{}.json".format(args.phase))
