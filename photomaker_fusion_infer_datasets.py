...
pipe.set_fusion_model(unet_path=None)
...
face_id_embeds = data['face_ids'][0].to("cuda").unsqueeze(dim=0)

neg_face_id_embeds = torch.zeros_like(face_id_embeds)
id_embeds = torch.stack([neg_face_id_embeds, face_id_embeds], dim=0).to(dtype=torch.float16, device="cuda")
# id_embeds = face_id_embeds
clip_embeds = data["image_embeds"][0].to("cuda").unsqueeze(dim=0)
neg_clip_embeds = torch.zeros_like(clip_embeds)
clip_embeds = torch.stack([neg_clip_embeds, clip_embeds], dim=0).to(dtype=torch.float16, device="cuda")
