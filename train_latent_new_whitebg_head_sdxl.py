...
from model.datasets.celebv_text import CelebVTextWhiteSD15Dataset,CelebVTextWhiteHeadSD15Dataset, CelebVTextWhiteHeadSDXLDataset
...
def collate_fn(data):
    ...
    pooled_prompt_embeds = torch.stack([example["pooled_prompt_embeds"] for example in data])
    prompt = [example["prompt"] for example in data]
    drop_image_flag = [example["drop_image"] for example in data]
    return {
        "video": videos,
        "prompt_embeds": prompt_embeds,
        "pooled_prompt_embeds": pooled_prompt_embeds,
        "prompt": prompt,
        "drop_image_flag": drop_image_flag,
    }
...
def main(args):
    ...
    train_dataset = CelebVTextWhiteHeadSDXLDataset(args.train_data_dir, video_length=args.video_length, resolution=[args.resolution,args.resolution],frame_stride=args.frame_stride, new_prompt=args.enable_new_prompt)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                ref_latents = batch["video"][:,:,-1:,:,:].to(weight_dtype)
