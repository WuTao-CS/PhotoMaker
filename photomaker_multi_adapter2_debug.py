...
for image_path,input_id_image in zip(image_path_list, input_id_images):
    ...
    if args.multi_ip_adapter:
        clip_embeds = pipe.prepare_ip_adapter_image_embeds([input_id_images[0],input_id_images[0]], None, torch.device("cuda"), 1, False)[0]
        torch.save(clip_embeds, "{}.ipadpt".format(image_path.split('.')[0]))
        
    
    id_embeds=id_embeds.to(dtype=clip_embeds.dtype,device=clip_embeds.device)
    ## Parameter setting
